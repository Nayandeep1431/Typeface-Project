...
export const createBulkTransactions = createAsyncThunk(
  ...
  async ({ transactions, source = 'bulk_upload' }, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/transactions/bulk`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
...
export const fetchTransactionStats = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/transactions/stats?${queryParams}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
